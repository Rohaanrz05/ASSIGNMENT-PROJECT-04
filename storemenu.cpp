...
struct MenuItem
{
    ...
    string toMenuItemString() const
    {
        return item_code + "," + name + "," + to_string(price) + "," + to_string(quantity_available);
    }
};
...
public:
    ...
    void saveMenu()
    {
        ...
        {
            file << item.toMenuItemString() << endl;
        }
        file.close();
...
int main()
{
    mainMenu(); // Calling the main menu function
